...
uniform float uHasYoshi;
uniform float uHasMario;
uniform float uHasGoomba;
uniform float uYoshiDensity;

vec3 SampleSky( float pixelX, float pixelY, float w, float h )
{
    // 与游戏像素网格对齐的屏幕空间采样
    vec2 uv = vec2( pixelX / w, 1.0 - pixelY / h );
    return texture2D( iChannel0, uv ).xyz;
}

vec3 SampleSpriteTex( sampler2D tex, float x, float y )
{
    return texture2D( tex, vec2( x / 16.0, ( 15.0 - y ) / 16.0 ) ).xyz;
}
...
// ===== "逃跑之后变安详" 结局时间线 =====
const float END_WALK_START   = 40.45;   // 马里奥从旗杆走向床
const float END_LIE_START    = 41.30;   // 躺到床上
const float END_ZZZ_START    = 43.00;   // 开始打 Zzz
const float PIGEON_FLY_START = 43.50;   // 鸽子开始飞
const float PIGEON_LAND      = 46.00;   // 鸽子落到肚子上
...
void SpriteGoomba( inout vec3 color, float x, float y, float frame )
{
    ...
    if ( uHasGoomba > 0.5 && idx > 0.0 )
    {
        color = SampleSpriteTex( iChannel3, x, y );
    }
}
...
{    
    ...
    if ( uHasMario > 0.5 && idx > 0.0 )
    {
        color = SampleSpriteTex( iChannel2, x, y );
    }    
}
...
void SpriteYoshi( inout vec3 color, float x, float y, vec3 body )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 21824.0 : 341.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 43328.0 : 1706.0 ) : idx;
    idx = y == 2.0 ? ( x <= 7.0 ? 43264.0 : 1706.0 ) : idx;
    idx = y == 3.0 ? ( x <= 7.0 ? 59648.0 : 1711.0 ) : idx;
    idx = y == 4.0 ? ( x <= 7.0 ? 59648.0 : 431.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 43344.0 : 431.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 43508.0 : 426.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 43517.0 : 1706.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 43517.0 : 24234.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 38564.0 : 32426.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 42896.0 : 6826.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 42896.0 : 1706.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 43584.0 : 426.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 43264.0 : 106.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 41984.0 : 90.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 20480.0 : 5.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
    color = idx == 1.0 ? RGB( 0, 0, 0 ) : color;
    color = idx == 2.0 ? body : color;
    color = idx == 3.0 ? RGB( 255, 255, 255 ) : color;
}

void SpriteMarioSleep( inout vec3 color, float x, float y )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 21824.0 : 21.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 65492.0 : 383.0 ) : idx;
    idx = y == 2.0 ? ( x <= 7.0 ? 64932.0 : 2047.0 ) : idx;
    idx = y == 3.0 ? ( x <= 7.0 ? 65193.0 : 7669.0 ) : idx;
    idx = y == 4.0 ? ( x <= 7.0 ? 63145.0 : 8191.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 64169.0 : 7679.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 55972.0 : 8157.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 21840.0 : 1365.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
    color = idx == 1.0 ? RGB( 0, 0, 0 ) : color;
    color = idx == 2.0 ? RGB( 231, 90, 16 ) : color;
    color = idx == 3.0 ? RGB( 247, 214, 181 ) : color;
...
void SpritePigeonFly1( inout vec3 color, float x, float y )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 2.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 3.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 4.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 21504.0 : 1.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 48384.0 : 6.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 44368.0 : 26.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 42660.0 : 106.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 43344.0 : 430.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 43264.0 : 426.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 43264.0 : 1686.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 41984.0 : 321.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 20480.0 : 0.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
    color = idx == 1.0 ? RGB( 20, 20, 30 ) : color;
    color = idx == 2.0 ? RGB( 150, 150, 165 ) : color;
    color = idx == 3.0 ? RGB( 245, 245, 250 ) : color;
}

void SpritePigeonFly2( inout vec3 color, float x, float y )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    ...
    idx = y == 4.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 21760.0 : 0.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 44864.0 : 1.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 43584.0 : 1.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 47760.0 : 6.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 43664.0 : 26.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 21924.0 : 105.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 105.0 : 420.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 20.0 : 80.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
    color = idx == 1.0 ? RGB( 20, 20, 30 ) : color;
...
void SpritePigeonStand( inout vec3 color, float x, float y )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 2.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 3.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 4.0 ? ( x <= 7.0 ? 320.0 : 0.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 2000.0 : 0.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 5456.0 : 0.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 28480.0 : 0.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 43584.0 : 6.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 44608.0 : 26.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 43584.0 : 22.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 43264.0 : 1.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 21504.0 : 0.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
...
void SpriteZ( inout vec3 color, float x, float y )
{
    float idx = 0.0;
    idx = y == 0.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 1.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    ...
    idx = y == 4.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 5.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 6.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 7.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 8.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 9.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 10.0 ? ( x <= 7.0 ? 0.0 : 0.0 ) : idx;
    idx = y == 11.0 ? ( x <= 7.0 ? 341.0 : 0.0 ) : idx;
    idx = y == 12.0 ? ( x <= 7.0 ? 20.0 : 0.0 ) : idx;
    idx = y == 13.0 ? ( x <= 7.0 ? 80.0 : 0.0 ) : idx;
    idx = y == 14.0 ? ( x <= 7.0 ? 320.0 : 0.0 ) : idx;
    idx = y == 15.0 ? ( x <= 7.0 ? 341.0 : 0.0 ) : idx;
    idx = SPRITE_DEC( x, idx );
    color = idx == 1.0 ? RGB( 255, 255, 255 ) : color;
}

// ===== 耀西背景 =====
// 注:不能用 sin(large)*43758 之类的哈希,GPU 上 sin 大参数精度会坍缩
float YoshiHash( float n ) { return fract( n * 0.6180339887 ); }
float YoshiHash2( float n ) { return fract( n * 0.7548776662 ); }

void DrawYoshis( inout vec3 color, float worldX, float worldY, float time )
{
    // 随进度(0->1)耀西逐渐消失:每只耀西有自己的阈值
    // 25s(音乐进入 Var.11)时全部消失
    float progress = clamp( ( time - 2.0 ) / 23.0, 0.0, 1.0 );
    float slotW    = floor( clamp( 36.0 / uYoshiDensity, 16.0, 360.0 ) );
    float slot     = floor( worldX / slotW );
    float lx       = mod( worldX, slotW ) - floor( ( slotW - 16.0 ) / 2.0 );
    float h1       = YoshiHash( slot + 11.0 );
    float h2       = YoshiHash2( slot + 3.0 );
    float baseY    = 72.0 + floor( h2 * 88.0 );
    float bob      = floor( sin( time * 2.0 + slot * 1.7 ) * 3.0 );
    float ly       = worldY - ( baseY + bob );
    if ( h1 >= progress * 1.04 && lx >= 0.0 && lx <= 15.0 && ly >= 0.0 && ly <= 15.0 )
    {
        if ( uHasYoshi > 0.5 )
        {
            vec4 t = texture2D( iChannel1, vec2( lx / 16.0, ( 15.0 - ly ) / 16.0 ) );
            if ( t.w > 0.5 ) { color = t.xyz; }
        }
        else
        {
            vec3 body = h2 < 0.25  ? RGB( 0,   200, 60  )
                      : h2 < 0.5   ? RGB( 230, 60,  60  )
                      : h2 < 0.75  ? RGB( 40,  80,  230 )
                      :              RGB( 255, 215, 0   );
            SpriteYoshi( color, lx, ly, body );
        }
    }
}

// ===== 结局:床 =====
void DrawBed( inout vec3 color, float x, float y )
{
    if ( x >= 0.0 && x < 48.0 && y >= 0.0 && y < 32.0 )
    {
        // 床腿
        if ( ( ( x >= 2.0 && x < 6.0 ) || ( x >= 42.0 && x < 46.0 ) ) && y < 14.0 )
        {
            color = RGB( 103, 44, 8 );
        }
        // 床架
        if ( y >= 14.0 && y < 17.0 && x >= 2.0 && x < 46.0 )
        {
            color = RGB( 146, 64, 14 );
        }
        // 床垫
        if ( y >= 17.0 && y < 26.0 && x >= 4.0 && x < 44.0 )
        {
            color = RGB( 255, 252, 240 );
        }
        // 床头板(左)
        if ( x >= 0.0 && x < 6.0 && y >= 17.0 )
        {
            color = RGB( 146, 64, 14 );
        }
        // 床尾板(右)
        ...
        // 枕头
        if ( y >= 24.0 && y < 30.0 && x >= 7.0 && x < 21.0 )
        {
            color = RGB( 255, 255, 255 );
        }
        // 枕头阴影
        if ( y == 24.0 && x >= 8.0 && x < 20.0 )
        {
...
// ===== 结局:被子(盖在马里奥身上) =====
void DrawBlanket( inout vec3 color, float x, float y )
{
    if ( x >= 0.0 && x < 24.0 && y >= 0.0 && y < 9.0 )
    {
        color = RGB( 255, 150, 180 );
        if ( y == 0.0 ) { color = RGB( 230, 110, 150 ); }
        if ( x == 23.0 ) { color = RGB( 230, 110, 150 ); }
        // 被子花纹
        if ( ( mod( x, 6.0 ) == 3.0 && mod( y, 4.0 ) == 2.0 ) )
        {
...
void DrawGame( inout vec3 color, float time, float pixelX, float pixelY, float screenWidth, float screenHeight )
{
    ...
    if ( uHasSky > 0.5 )
    {
        color = SampleSky( pixelX, pixelY, screenWidth, screenHeight );
    }

    // 背景耀西(随进度减少,消失后露出蓝天)
    DrawYoshis( color, worldX, worldY, time );
    ...
    // ===== 结局场景:床 + 睡觉的马里奥 + 鸽子 =====
    DrawBed( color, worldX - 3236.0, worldY - 16.0 );

    // 马里奥从旗杆走向床
    if ( time >= END_WALK_START && time < END_LIE_START )
    {
        float mw = 3160.0 + floor( ( time - END_WALK_START ) * MARIO_SPEED );
        float mx = pixelX - ( mw - cameraX );
        float my = pixelY - 16.0 - 8.0;
        float mf = floor( mod( time * 10.0, 3.0 ) ) + 4.0;
        if ( mx >= 0.0 && mx <= 15.0 )
        {
            SpriteMario( color, mx, my, mf );
        }
    }

    // 马里奥躺下睡觉(枕在枕头上)
    if ( time >= END_LIE_START )
    {
        float sx = pixelX - ( 3241.0 - cameraX );
        float sy = pixelY - 8.0 - 40.0;
        if ( sx >= 0.0 && sx <= 15.0 )
        {
            SpriteMarioSleep( color, sx, sy );
        }
        // 被子盖在身上
        DrawBlanket( color, pixelX - ( 3256.0 - cameraX ), pixelY - 8.0 - 36.0 );
    }

    // Zzz
    if ( time >= END_ZZZ_START )
    {
        for ( int i = 0; i < 3; i++ )
        {
            float fi = float( i );
            float phase = fract( ( time - END_ZZZ_START ) * 0.4 + fi * 0.33 );
            float zx = pixelX - ( 3262.0 + fi * 5.0 - cameraX );
            float zy = pixelY - 8.0 - ( 56.0 + floor( phase * 14.0 ) );
            if ( zx >= 0.0 && zx <= 15.0 )
            {
                SpriteZ( color, zx, zy );
            }
    ...
    // 鸽子:从右上飞进来,落到马里奥肚子上
    if ( time >= PIGEON_FLY_START )
    {
        float u  = clamp( ( time - PIGEON_FLY_START ) / ( PIGEON_LAND - PIGEON_FLY_START ), 0.0, 1.0 );
        u = u * u * ( 3.0 - 2.0 * u );
        float pw = mix( 3420.0, 3256.0, u );
        float pwy = mix( 200.0, 44.0, u ) + sin( u * 3.14 ) * 24.0;
        float pf = 0.0;
        if ( time >= PIGEON_LAND )
        {
            pw  = 3256.0;
            pwy = 44.0 + floor( mod( time * 2.0, 2.0 ) );
            pf  = 2.0;
        }
        else
        {
            pf = floor( mod( time * 10.0, 2.0 ) );
        }
        float px  = pixelX - ( pw - cameraX );
        float pyy = pixelY - 8.0 - pwy;
        if ( px >= 0.0 && px <= 15.0 && pyy >= 0.0 && pyy <= 15.0 )
        {
            if ( pf == 0.0 )      { SpritePigeonFly1( color, px, pyy ); }
            else if ( pf == 1.0 ) { SpritePigeonFly2( color, px, pyy ); }
            else                  { SpritePigeonStand( color, px, pyy ); }
        }
    }
    ...
    if ( marioX >= 0.0 && marioX <= 15.0 && cameraX < 3152.0 && time < END_WALK_START )
    {
